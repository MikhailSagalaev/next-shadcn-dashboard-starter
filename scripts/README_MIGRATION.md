...
- `Сумма покупок` / `Общая сумма` / `AlltimeCost` → totalPurchases
- `Количество бонусов` → текущий баланс бонусов (создается транзакция EARN)
- `Дата первой покупки` → registeredAt
- `Статус` / `Статус клиента` / `Status` → isActive
- `CashbackLevel` / `tilda_level` → определяет уровень пользователя
- `ref_link` → реферальная ссылка
- `ID` → ID записи из Airtable (сохраняется в метаданных)
...
### Рекомендуемые поля (стандартный формат):
```csv
...
### Поля из Airtable (реальный формат):
```csv
ID,Email,Имя,Количество бонусов,Status,AlltimeCost,CashbackLevel,tilda_level,ref_link
rec123,test@example.com,Иван,500,активный,15000,2,Уровень 2. СЕРЕБРЯНЫЙ,https://site.ru/?ref=rec123
rec456,user@test.ru,Мария,1200,активный,25000,3,Уровень 3. ЗОЛОТОЙ,https://site.ru/?ref=rec456
```
...
### Проблемы с Airtable CSV:
- **Кодировка**: CSV из Airtable может содержать кириллицу в неправильной кодировке
- **Многострочные скрипты**: Поля с JavaScript кодом ломают CSV структуру
- **Специальные символы**: Кавычки и запятые в скриптах могут сломать парсинг

**Решение**: Перед миграцией проверьте и очистите CSV файл от проблемных символов.

### Проверка CSV файла:
```bash
# Просмотр первых строк файла
head -10 customers.csv

# Подсчет строк в файле
wc -l customers.csv

# Проверка на наличие проблемных символов
grep -n '"' customers.csv | head -5
```
...
```
Решение: Перекодируйте CSV файл в UTF-8
```

### Проблема: "Многострочные скрипты ломают CSV"
```
Решение: Удалите или замените многострочные скрипты в CSV перед миграцией
```

### Проблема: "Не распознаются поля из Airtable"
```
Решение: Скрипт поддерживает поля: Email, Имя, Количество бонусов, Status, AlltimeCost, CashbackLevel, tilda_level, ref_link
```
