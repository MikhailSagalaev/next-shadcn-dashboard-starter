## [2025-09-29] - Автоматический пересчет бонусов при изменении корзины + Статистика пользователей проекта
...
- **Новая версия виджета**: Обновлена до версии 1.1.0 (v6) с автоматическим пересчетом бонусов

### 🔧 Исправления интеграции с Tilda
- **КРИТИЧНО**: Исправлена проблема с промокодами при изменении корзины - бонусы теперь автоматически пересчитываются при удалении товаров
- **КРИТИЧНО**: Добавлена функция `adjustBonusesForCartChange()` для автоматической корректировки примененных бонусов
- **КРИТИЧНО**: Виджет теперь отслеживает все события изменения корзины (`tcart:updated`, `tcart:quantity:changed`, `tcart:recalculated`)
- **КРИТИЧНО**: При превышении доступного максимума бонусов система автоматически корректирует сумму до допустимого значения
- **УЛУЧШЕНО**: Добавлено информационное уведомление пользователю при автоматической корректировке бонусов
- **УЛУЧШЕНО**: Улучшена совместимость с различными типами корзин Tilda

### 🔄 Изменено
- Обновлена версия виджета с v5 до v6 для применения автоматического пересчета бонусов
- Заменены статистические карточки на странице пользователей проекта на компонент `BonusStatsCards`

### 🐛 Исправлено
- Бонусы больше не "застревают" при уменьшении суммы заказа - автоматически пересчитываются до доступного максимума
