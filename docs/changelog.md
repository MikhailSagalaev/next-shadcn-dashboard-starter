...
### 🎯 Добавлено
- **Выбор типа приветственного вознаграждения** в настройках реферальной программы
- **Тип BONUS** — начисление фиксированной суммы бонусов при регистрации (текущее поведение)
- **Тип DISCOUNT** — процентная скидка на первую покупку нового пользователя
- **Поле `firstPurchaseDiscountPercent`** — процент скидки (0-100%)
- **Информация о скидке в webhook ответе** — при первой покупке возвращается `firstPurchaseDiscount`

### 🔄 Изменено
- **UI настроек реферальной программы** — добавлен переключатель между бонусами и скидкой
- **Боковая панель** — отображает выбранный тип вознаграждения

### 📝 Миграция БД
```sql
CREATE TYPE "WelcomeRewardType" AS ENUM ('bonus', 'discount');
ALTER TABLE "referral_programs" ADD COLUMN "welcome_reward_type" "WelcomeRewardType" DEFAULT 'bonus';
ALTER TABLE "referral_programs" ADD COLUMN "first_purchase_discount_percent" INTEGER DEFAULT 0;
```
