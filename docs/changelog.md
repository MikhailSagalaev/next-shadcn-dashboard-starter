...
- **УЛУЧШЕНО**: Добавлены недостающие функции Tilda для лучшей совместимости:
  - Добавлена `tcart__calcAmountWithDiscounts()` для пересчета сумм со скидками
  - Добавлена `tcart__updateTotalProductsinCartObj()` для обновления объектов товаров
  - Теперь используется полный набор функций обновления корзины Tilda
