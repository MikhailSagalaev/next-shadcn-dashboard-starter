...
## [2026-07-18] - B2B UX, организации и безопасный учёт заказов

### 🎯 Добавлено
- Серверная пагинация B2B-combobox, перенос участников между организациями и полный row menu: редактирование, копирование реферальной ссылки, перенос и удаление.
- Идемпотентный скрипт `scripts/update-b2b-partner-workflows.ts` с `--projectId` и `--dry-run`; новая `WorkflowVersion` создаётся только при структурных изменениях.
- Состояния учёта заказа и claim-защита конкурентных начислений/возвратов; миграции `20260718190000_b2b_orders_accounting` и `20260718200000_archive_referral_plans`.

### 🔄 Изменено
- Пользовательские роли унифицированы как «Клиент», «Уровень 1», «Уровень 2», «Уровень 3» без изменения Prisma enum.
- Наличные заказы создаются `PENDING` с `paidAmount = 0`; бонусная экономика применяется только при ручном переходе в `CONFIRMED`.
- RetailCRM проводит статусы последовательно через accounting boundary; отмена и возврат выполняют идемпотентный reversal и обновляют оборот/уровень пользователя.
- Используемые партнёрские планы архивируются (`isActive = false`), неиспользованные удаляются; default/outbound назначения принимают только активные планы.
- B2B Telegram workflow использует кнопку `👉 МОЯ ССЫЛКА 👈` и жирный заголовок ответа.
- Tilda first-touch атрибуция очищается только после успешного события `tildaform:aftersuccess`; вход Tilda/Safari и ПВЗ не изменялись.

### 🐛 Исправлено
- Устранены наследование старой `utm_ref` следующей регистрацией, ограниченная первая страница выбора пользователя, неверный выбор руководителя, legacy toast и удаление используемых планов.
- FIFO-восстановление списанных bonus lots больше не увеличивает сумму повторно; legacy purchase/referral начисления безопасно компенсируются при отмене.
- `ReferralService.getReferralStats` устойчив к отсутствующему mock-результату `groupBy`.

### ✅ Проверено
- `npx prisma validate` и `npx prisma generate` — успешно.
- Referral Jest suite — 14/14; `node --check public/tilda-bonus-widget.js` — успешно.
- Targeted ESLint всех изменённых TS/TSX — 0 ошибок; IDE diagnostics — без ошибок; Prettier — чисто.
- `yarn build` — успешно, exit code 0. Полный `npx tsc --noEmit` по-прежнему блокируется 91 накопленной ошибкой в 27 baseline-файлах МойСклад/InSales/Next validators; новые ошибки workflow-скрипта и PATCH плана устранены.

---

## [2026-07-18] - Автономный SSH deploy закреплён в правилах проекта

### 🔄 Изменено
- Явная просьба выполнить production deploy теперь зафиксирована как разрешение агенту автономно пройти весь конкретный deploy: SSH-подключение, безопасное обновление `main`, миграции/Prisma generate при необходимости, единственную Webpack-сборку, PM2 restart, health-check и проверку startup-логов.
- В workspace steering добавлены production-контекст проекта, сохранение server-side изменений, запрет автоматического изменения `.env`, проверка `.next/BUILD_ID`, правила low-memory build и безопасного PowerShell/SSH quoting.
- Канонический VPS-гайд дополнен фактическим сценарием для `bonus-app` и fallback через минимальный Git bundle при недоступном remote Git.
- Troubleshooting дополнен диагностикой зависшего/параллельного build и требованием завершать только конкретные PID.

### ✅ Проверено
- Изменения ограничены документацией и workspace steering; production-команды не запускались.
- `docs/README.md` уже содержит ссылку на канонический VPS guide, отдельный дублирующий документ не создавался.

---

## [2026-07-18] - Автоматическое отключение недоступных Telegram-привязок

### 🐛 Исправлено
- Реальные Telegram API `403` с причинами `bot was blocked by the user` и `user is deactivated` теперь очищают устаревший `telegramId`; такие пользователи автоматически исключаются из следующих рассылок.
- Бонусный аккаунт пользователя остаётся активным, `telegramUsername` сохраняется, а повторная привязка после разблокировки остаётся возможной.
- Очистка защищена условиями `id + projectId + прежний telegramId`, поэтому поздний ответ Telegram не удалит новую параллельно созданную привязку; внутренние 403 проверки проекта и бота не затрагивают пользователя.

### ✅ Проверено
- Production-логи подтвердили обработку всех 370 jobs, успешные доставки и немедленную фиксацию permanent 403 без retry.
- IDE diagnostics — без ошибок; targeted ESLint — 0 ошибок; `yarn build` — exit code 0.
...
### 🎯 Добавлено
- Фоновая команда «Рассылка всем» через существующую BullMQ-очередь: создаётся `Mailing`, eligible-получатели ставятся в очередь блоками по 500, API сразу отвечает `202 Accepted`.
- Миграция `20260718140000_add_admin_notifications` для enum, таблицы, индексов и внешних ключей `admin_notifications`.
- Раздельная статистика доставки `sent / failed / skipped`; пользователи без активной Telegram-привязки больше не считаются ошибками Telegram.

### 🔄 Изменено
- Telegram-рассылка выбранным пользователям выполняется одной пакетной операцией и больше не дублируется в MAX.
- BullMQ использует общий разбор `REDIS_URL`/`REDIS_HOST`, worker запускается лениво, ограничен 15 jobs/sec и завершает `Mailing` после обработки всех получателей.
- Временные Telegram-ошибки (`429`, `5xx`, network) передаются BullMQ для трёх попыток с backoff; `retry_after` учитывается перед повтором. Неисправимые ошибки фиксируются сразу, отменённые рассылки не продолжают отправку.
- Сообщение с изображением и текстом длиннее Telegram caption-limit отправляется как изображение и отдельное текстовое сообщение.
- Таблица ограничена страницами 10/20/50/100, использует стабильный row ID и сбрасывает selection при смене серверной страницы/фильтров. Общий счётчик проекта отделён от количества строк текущего фильтра.
- Prisma-запрос списка больше не загружает полный проект и массив referrals; данные реферера ограничены отображаемыми полями. Экспорт получает пользователей страницами по 100 без повторного расчёта статистики.

### 🐛 Исправлено
- Причина результата «5 отправлено / 995 ошибок»: из 1000 строк текущей страницы только 5 пользователей имели Telegram, остальные теперь корректно учитываются как `skipped`.
- Кнопка «Выбрать все» переименована в «Выбрать страницу», а глобальная аудитория вынесена в отдельную «Рассылка всем» без лимита 1000.
- `db.adminNotification.findMany` больше не зависит от отсутствующей миграции; после deploy миграции и регенерации Prisma Client модель доступна runtime.

### ✅ Проверено
- `npx prisma validate` — успешно; `npx prisma generate` — успешно.
- `yarn build` — успешно; локально зафиксированы предупреждения о недоступных Redis и PostgreSQL `127.0.0.1:5440`, итоговый exit code 0.
- IDE diagnostics изменённых файлов — без ошибок; targeted ESLint — 0 ошибок (остались накопленные warnings проекта).
- Полный `npx tsc --noEmit` блокируется существующими ошибками в МойСклад Loyalty, InSales-скриптах и proxy module resolution; новых ошибок в изменённых файлах нет.
- Реальная Telegram-отправка не запускалась: после деплоя требуется smoke test с настроенным Redis и активным ботом.

### 🚀 Деплой
- Вручную настроить `REDIS_URL` или `REDIS_HOST` (env-файлы автоматически не изменялись).
- Выполнить: `npx prisma migrate deploy`, `npx prisma generate`, `yarn build`, затем `yarn pm2:restart`.

---

## [2026-07-18] - Аудит Telegram-рассылок и диагностики доставки

### 🎯 Добавлено
- Технический аудит `docs/telegram-mailing-delivery-audit.md`: end-to-end карта трёх контуров отправки, места хранения ошибок, причины недоставки конкретному пользователю и практический чек-лист диагностики.
- Сверка с Telegram Bot API 10.2 и актуальным grammY 1.45.1; рекомендации по `retry_after`, `@grammyjs/auto-retry`, очередям и paid broadcasts.

### ⚠️ Выявлено
- Mailing BullMQ worker не инициализируется; `scheduledAt` не обслуживается планировщиком; строковый `REDIS_URL` заменяется на localhost.
- Telegram `429/5xx/network` не приводят к retry; cancel не останавливает jobs; status `COMPLETED` выставляется только при чтении аналитики.
- Ошибки распределены между `mailing_recipients`, `mailing_history`, `workflow_*`, `system_logs` и runtime console; прямые уведомления не имеют полноценного delivery audit trail.

### 🔄 Изменено
- В `docs/README.md` добавлена ссылка на аудит.
- Production-код и схема БД не изменялись; исправления разбиты на приоритетные фазы P0–P2.
