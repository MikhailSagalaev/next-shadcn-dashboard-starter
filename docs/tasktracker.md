...
## 📋 Задача: B2B UX организаций и ручное подтверждение наличных заказов
- **Статус**: ✅ Завершена
- **Приоритет**: 🔴 Высокий
- **Описание**: Исправлено управление B2B-организациями и партнёрами, пользовательские названия унифицированы как «Уровень 1/2/3 + Клиент», реферальная ссылка сделана заметной, используемые планы безопасно архивируются, а наличные заказы учитываются только после ручного подтверждения. Вход Tilda/Safari и обязательность ПВЗ не изменялись.
- **Техническая сложность**: 5
- **Затраченное время**: 8 часов
- **Шаги выполнения**:
  - [x] Унифицировать отображаемые названия ролей без изменения Prisma enum
  - [x] Добавить поиск с догрузкой всех пользователей, row menu, копирование реф-ссылки и перенос между организациями
  - [x] Исправить назначение руководителя, сохранение реферера/плана и success-feedback через Sonner
  - [x] Очищать first-touch атрибуцию после успешной конверсии, чтобы следующий ручной signup не наследовал старую организацию
  - [x] Архивировать используемые партнёрские планы и удалять только неиспользованные
  - [x] Обновить B2B workflow: заметная кнопка `👉 МОЯ ССЫЛКА 👈` и жирный заголовок ответа
  - [x] Наличные заказы оставлять PENDING без начислений до ручного CONFIRMED; отмену/возврат проводить идемпотентно
  - [x] Проверить TypeScript, lint, Prisma и production build
  - [x] Обновить документацию и changelog
- **Зависимости**: B2B hierarchy, PartnerOrganization, ReferralCommissionPlan, Tilda order webhook, Order/Bonus/Transaction ledger, Telegram workflow versions
- **Отложено**: отдельная архитектурная задача — настраиваемая per-organization пирамида с произвольным числом уровней (до 100 и более), без привязки к фиксированному enum.
- **Тестирование**: `npx prisma validate` и `npx prisma generate` — успешно; referral Jest — 14/14; targeted ESLint — 0 ошибок; diagnostics и Prettier — чисто; `node --check public/tilda-bonus-widget.js` — успешно; `yarn build` — exit 0. Полный `npx tsc --noEmit` фиксирует 91 накопленную baseline-ошибку в 27 старых файлах МойСклад/InSales/Next validators; две новые ошибки текущего набора устранены. Для Tilda остаётся ручной browser smoke test `tildaform:aftersuccess`; workflow-скрипт запускать только с конкретным `projectId`.

---

## 📋 Задача: Закрепить автономный production deploy в правилах проекта
- **Статус**: ✅ Завершена
- **Приоритет**: 🔴 Высокий
- **Описание**: Зафиксирован одобренный пользователем автономный SSH deploy: при явной просьбе применить релиз в production агент самостоятельно выполняет безопасное обновление, сборку, перезапуск PM2 и smoke-check, не перекладывая доступные операции на пользователя.
- **Техническая сложность**: 2
- **Затраченное время**: 0.5 часа
- **Шаги выполнения**:
  - [x] Проанализировать текущие workflow/testing rules и deployment-документацию
  - [x] Добавить постоянные правила production deploy и безопасного SSH quoting
  - [x] Описать project-specific PM2/Webpack deploy и fallback через Git bundle
  - [x] Добавить troubleshooting зависшей или параллельной сборки
  - [x] Обновить changelog и проверить Markdown
- **Зависимости**: SSH-доступ, Git, Yarn, Next.js Webpack build, PM2, production health endpoint
- **Тестирование**: Diagnostics всех затронутых Markdown/steering-файлов — без ошибок; `git diff --check` — exit code 0. Production-команды в рамках документационной задачи не запускались.
...
- **Статус**: ✅ Завершена
- **Приоритет**: 🔴 Высокий
- **Описание**: Исключена отправка Telegram-сообщений непривязанным пользователям и ошибочное дублирование в MAX, рассылка всем перенесена в фоновую очередь без лимита 1000, таблица и Prisma-запрос облегчены, добавлена миграция admin notifications.
- **Техническая сложность**: 4
- **Затраченное время**: 3 часа
- **Шаги выполнения**:
  - [x] Фильтровать получателей по активной Telegram-привязке и показывать число пропущенных
  - [x] Выполнять рассылку всем через BullMQ без HTTP-таймаута
  - [x] Добавить retry transient Telegram-ошибок и учитывать `retry_after`
  - [x] Ограничить размер страницы таблицы и облегчить Prisma-запрос
  - [x] Разделить общий счётчик проекта и количество пользователей под текущим фильтром
  - [x] Добавить миграцию `admin_notifications` и обновить Prisma Client
  - [x] Проверить TypeScript, lint и Prisma schema
- **Зависимости**: Redis для фоновой очереди; активный Telegram-бот проекта; пользователи без Telegram учитываются как `skipped`
- **Тестирование**: `npx prisma validate` — успешно; `npx prisma generate` — успешно; `yarn build` — успешно (exit 0, локальные предупреждения из-за выключенных Redis/PostgreSQL); diagnostics — чисто; targeted ESLint — 0 ошибок. Полный `npx tsc --noEmit` остаётся заблокирован существующими ошибками в других модулях, изменённые файлы ошибок не добавляют.
- **После деплоя**: настроить `REDIS_URL`/`REDIS_HOST`; выполнить `npx prisma migrate deploy`, `npx prisma generate`, `yarn build`, `yarn pm2:restart`; проверить `202`, статусы `SENDING → COMPLETED`, retry и `GET /api/notifications/admin`.
...
## 📋 Задача: Аудит Telegram-рассылок и диагностики доставки
- **Статус**: ✅ Завершена
- **Приоритет**: 🔴 Высокий
- **Описание**: Проследить все контуры массовых Telegram-отправок, определить места хранения ошибок и причины недоставки отдельным пользователям, сверить реализацию с Telegram Bot API 10.2 и актуальным grammY.
- **Техническая сложность**: 4
- **Затраченное время**: 1.5 часа
- **Шаги выполнения**:
  - [x] Найти точки запуска, выбора аудитории и фактической отправки
  - [x] Проверить BullMQ worker, Redis, retry, rate limiting и планировщики
  - [x] Проверить модели БД, системные логи и UI аналитики
  - [x] Сверить с актуальными требованиями Telegram/grammY
  - [x] Оформить выводы, приоритеты исправлений и сценарий диагностики
- **Зависимости**: Telegram Bot API, grammY, BullMQ, Redis, PostgreSQL
- **Результат**: `docs/telegram-mailing-delivery-audit.md`; выявлены 4 P0 и 14 P1 рисков, предложен поэтапный план исправлений.
- **Тестирование**: Статический repository-wide аудит, сверка с официальными Telegram/grammY источниками, проверка ссылок и диагностика изменённых Markdown-файлов. Production-код не изменялся.

---

## 📋 Задача: Production-исправление Telegram-рассылок
- **Статус**: 🔄 В процессе
- **Приоритет**: 🔴 Высокий
- **Описание**: Устранить риски аудита Telegram-рассылок: запуск worker и scheduler, Redis URL, retry/rate limits, идемпотентность, корректные статусы/отмена, delivery observability и мультитенантные проверки.
- **Техническая сложность**: 5
- **Шаги выполнения**:
  - [ ] Исправить lifecycle BullMQ worker и Redis connection
  - [ ] Добавить scheduler due-кампаний и идемпотентный start
  - [ ] Унифицировать Telegram retry/error classification
  - [ ] Исправить cancel/completion и delivery audit trail
  - [ ] Усилить проверки project ownership
  - [ ] Улучшить API/UI диагностики получателей и очереди
  - [ ] Обновить Prisma schema/migration и зависимости
  - [ ] Выполнить TypeScript, lint, Prisma validate/generate и целевые тесты
  - [ ] Обновить документацию и changelog
- **Зависимости**: Telegram Bot API 10.2, grammY, BullMQ, Redis, PostgreSQL
- **Тестирование**: Будет заполнено после реализации.
