...
- Боты запускаются только для проектов в режиме `WITH_BOT`; при переключении проекта в `WITHOUT_BOT` активные боты останавливаются, но настройки сохраняются для обратного включения.
